...
within MoBTES.Components.Weather;
model WeatherData "Weather data (experimental)"
	MoBTES.Utilities.Interfaces.Weather weatherPort "Weather data connector" annotation(Placement(
		transformation(extent={{186.7,-10},{206.7,10}}),
		iconTransformation(extent={{186.7,-10},{206.7,10}})));
	Modelica.SIunits.Angle declinationAngle "Declination of the earth";
	Modelica.SIunits.Angle hourAngle "Hour angle";
	Modelica.SIunits.Angle zenitAngle "Zenith angle of the sun";
	Modelica.SIunits.Irradiance beamIrradiation[3] "Beam radiation vector";
	Modelica.SIunits.Irradiance beamIrradiationData "Beam radiation on a horizontal";
	Modelica.SIunits.Irradiance totalIrradiation "Total radiation";
	...
	protected
		...
		parameter Modelica.SIunits.Time timeOffset(
			fixed=true,
			start=(daysToEndOfMonth[initialMonth]-daysPerMonth[initialMonth]+initialDayOfMonth-1)*3600*24) "Startpoint of the simulation within a year in seconds";
	public
		parameter Integer initialMonth(
			min=1,
			max=12)=1 "Initial month of simulation";
		parameter Integer initialDayOfMonth(
			min=1,
			max=daysPerMonth[initialMonth])=1 "Initial day of month of the simulation";
		parameter Modelica.SIunits.Angle latitude=0.8642870455875921 "Latitude of location";
		parameter String weatherData=Modelica.Utilities.Files.loadResource("modelica://MoBTES/Data/Weather/Darmstadt_TRY2015.txt") "File where weather data is stored" annotation(Dialog(
			enable=tableOnFile,
			loadSelector(
				filter="Text files (*.txt);;MATLAB MAT-files (*.mat)",
				caption="Open file in which table is present")));
		parameter Boolean directDataOnHorizontal=true "=true, if direct radiation data is given in horizonatal direction (set to false for energy+ data).";
		Modelica.Blocks.Tables.CombiTable1D beamIrradiationTable(
			tableOnFile=true,
			tableName="beamIrradiation",
			fileName=weatherData) annotation(Placement(transformation(extent={{-55,55},{-35,75}})));
		Modelica.Blocks.Tables.CombiTable1D diffuseIrradiationTable(
			tableOnFile=true,
			tableName="diffuseIrradiation",
			fileName=weatherData) annotation(Placement(transformation(extent={{-55,55},{-35,75}})));
		Modelica.Blocks.Tables.CombiTable1D TambientTable(
			tableOnFile=true,
			tableName="Tambient",
			fileName=weatherData) annotation(Placement(transformation(extent={{-55,55},{-35,75}})));
	algorithm
	...
	equation
		//load weather data from file
			diffuseIrradiationTable.u[1]=mod(time+timeOffset,8760*3600)/3600;
			diffuseIrradiation=diffuseIrradiationTable.y[1];
			beamIrradiationTable.u[1]=mod(time+timeOffset,8760*3600)/3600;
			beamIrradiationData=beamIrradiationTable.y[1];
			totalIrradiation=if directDataOnHorizontal then beamIrradiationData+diffuseIrradiation else beamIrradiationData*cos(zenitAngle)+diffuseIrradiation;
			TambientTable.u[1]=mod(time+timeOffset,8760*3600)/3600;
			Tambient=TambientTable.y[1]+273.15;
		...
		//calculation of the beam radiation vector from horizontal measurements
			beamIrradiation=if zenitAngle > Modelica.Constants.pi/2*0.99 then {0,0,0} elseif directDataOnHorizontal then beamIrradiationData/cos(zenitAngle)*{-sin(hourAngle)*sin(zenitAngle),-cos(hourAngle)*sin(zenitAngle),cos(zenitAngle)} else beamIrradiationData*{-sin(hourAngle)*sin(zenitAngle),-cos(hourAngle)*sin(zenitAngle),cos(zenitAngle)} ;
...
<h4>General</h4>
<p>The weather data component reads annual temperature and irradiation data from text files and transfers them into a 3D radiation vector for a given location. </p>
<h5>Weather data files</h5>
<p>The weather data file should contain time series for:</p>
<ul>
<li>Ambient temperature (\"Tambient\")</li>
<li>Beam irradiation (\"beamIrradiation\")</li>
<li>Diffuse irradition (\"diffuseIrradiation\")</li>
</ul>
<p>Some data sources contain beam radiation measured on a horizontal surface (e.g. German Test Reference Years) and some in beam direction (e.g. energy+). The parameter <strong>directDataOnHorizontal</strong> has to be set accordingly.</p>

<h4>Implementation</h4>
<p>The model has not yet been properly validated and should therefor be regarded as experimental. Leap years are not yet implemented. For the locations Reykjavik, Teheran and Sydney the total irradiation on a sloped surface has been compared to the respective results from TRNSYS simulations <a href=\"modelica://MoBTES.UsersGuide.References\">[TRNSYS2017]</a>, with only minor deviations (Fig. 1).</p>
<table border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
  <caption align=\"bottom\"><strong>Fig. 1: </strong>Comparison of the annual total irradiation on a sloped solar thermal collector for three different locations.</caption>
  <tr>
    <td>
   <img src=\"modelica://MoBTES/Utilities/Images/IrradiationTRNSYScomparison.PNG\" width=\"1000\">
    </td>
  </tr>
</table>
</html>"),
