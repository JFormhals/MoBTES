...
model LocalElement_FiniteDifferences "Finite differences method model"
	...
	Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow heatFlowToLocal[nRings];
	Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow heatFlowToGlobal;
	...
	parameter Real thermalResistances[nRings]=cat(1,{log(massCenterRadii[1]/rBorehole)/(2*Modelica.Constants.pi*elementHeight*groundData.lamda)},{log(massCenterRadii[iRing+1]/massCenterRadii[iRing])/(2*Modelica.Constants.pi*elementHeight*groundData.lamda)for iRing in 1:nRings-1});
	equation
		Tmean=sum(volumeHeatCapacity[iRing].T*weights[iRing] for iRing in 1:nRings);
			
			local2globalPort.T=Tmean;
			heatFlowToGlobal.Q_flow=-Qflow*numberOfBHEsInRing;
			connect(heatFlowToGlobal.port,local2globalPort);
			for iRing in 1:nRings loop
				heatFlowToLocal[iRing].Q_flow=weights[iRing]*Qflow;
				connect(heatFlowToLocal[iRing].port,volumeHeatCapacity[iRing].port);
