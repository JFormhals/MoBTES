...
model CylinderElement_FiniteDifferences "Finite differences method model with port at outer boundary"
	...
	Modelica.Thermal.HeatTransfer.Components.HeatCapacitor volumeHeatCapacity[nRings](
		C=groundData.cp*groundData.rho*Modelica.Constants.pi*elementHeight*(rEquivalent^2-rBorehole^2)*weights,
		T(
			each start=Tinitial,
			each fixed=true)) "Lumped thermal element storing heat" annotation(Placement(transformation(extent={{-25,15},{-5,35}})));
	...
	parameter Real thermalResistances[nRings+1]=cat(1,
	{log(massCenterRadii[1]/rBorehole)/(2*Modelica.Constants.pi*elementHeight*groundData.lamda)},
	{log(massCenterRadii[iRing+1]/massCenterRadii[iRing])/(2*Modelica.Constants.pi*elementHeight*groundData.lamda)for iRing in 1:nRings-1},
	{log(radii[nRings+1]/massCenterRadii[nRings])/(2*Modelica.Constants.pi*elementHeight*groundData.lamda)}
	);
	equation
...
MXZqxpKMAAAAAElFTkSuQmCC",
						...
						Bitmap(
							imageSource="iVBORw0KGgoAAAANSUhEUgAAAOYAAADaCAYAAACy0j5tAAAABGdBTUEAALGPC/xhBQAAAAlwSFlz
...
hQyCvacZAAAAAElFTkSuQmCC",
							extent={{-66.8,-83.3},{66.40000000000001,66.59999999999999}})}),
