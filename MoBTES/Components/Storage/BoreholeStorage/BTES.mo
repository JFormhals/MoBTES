...
model BTES "BTES model"
	extends BaseClasses.partialBTES(Tstorage(displayUnit="°C") "Average temperature inside storage volume");
	...
	Modelica.Blocks.Sources.RealExpression TsurfaceAverage(y=location.Taverage)if settingsData.useAverageSurfaceTemperature annotation(Placement(transformation(extent={{-10,-10},{10,10}})));
	parameter MoBTES.Utilities.Types.BTESlayout BTESlayout=MoBTES.Utilities.Types.BTESlayout.hexagon "BTES layout" annotation(Dialog(
		group="Dimensions",
		groupImage=if Integer(BTESlayout)==1 then "modelica://MoBTES/Utilities/Images/BTESlayout1.png"else if Integer(BTESlayout)==2 then "modelica://MoBTES/Utilities/Images/BTESlayout2.png"else "modelica://MoBTES/Utilities/Images/BTESlayout3.png",
		tab="Design"));
	...
	parameter Modelica.SIunits.Length BHEspacing(
		...
		group="Dimensions",
		tab="Design"));
	replaceable model BHEmodel = MoBTES.Components.Sources.Geothermal.BaseClasses.DoubleUsegment constrainedby MoBTES.Components.Sources.Geothermal.BaseClasses.BHEsegment_partial "BHE type" annotation(
		choicesAllMatching=true,
		Placement(transformation(extent={{-10, -10}, {10, 10}})),
		Dialog(
			tab="Design",
			group="Borehole Heat Exchangers"));
	replaceable model GLOBALmodel = MoBTES.Components.Ground.GlobalElement_circular constrainedby MoBTES.Components.Ground.BaseClasses.GlobalElement_partial "Global problem modeling approach" annotation(
		choicesAllMatching=true,
		Dialog(
			group="Global problem",
			tab="Modeling"));
	replaceable model LOCALmodel = MoBTES.Components.Ground.LocalElement_steadyFlux constrainedby MoBTES.Components.Ground.BaseClasses.LocalElement_partial "Local problem modeling approach" annotation(
		choicesAllMatching=true,
		Dialog(
			group="Local problem",
			tab="Modeling"));
	protected
		...
		parameter Integer nBHEelementsR=if nSeries == 1 then MoBTES.Components.Storage.BoreholeStorage.Functions.NumberOfBHErings(nBHEs) else nSeries "Number of rings containg a BHE";
		...
		parameter Integer nBHEsPerRingVector[nBHEelementsR]=if nSeries == 1 then MoBTES.Components.Storage.BoreholeStorage.Functions.NumberOfBHEsPerRing(nBHEs, nBHEelementsR) else fill(integer(nBHEs / nBHEelementsR), nBHEelementsR) "Vector containing the number of BHEs per ring";
		...
		parameter Real GLOBAL_groundDataMatrix[3,nElementsZ,nElementsR]=MoBTES.Components.Storage.BoreholeStorage.Functions.ElementGroundData(heights, nElementsR, location) "Matrix containg thermal properties for each element of the global problem {rho,cp,lamda}" annotation(Evaluate=true);
		...
		Sources.Geothermal.BaseClasses.BHEhead BHE_Heads[nBHEelementsR](
			each BHEdata=BHEdata,
			each medium=medium,
			multiplicationFactor=nBHEsPerRingVector,
			each Tinitial=location.Taverage,
			each BHElength=BHElength) annotation(Placement(transformation(extent={{-10,15},{10,35}})));
		replaceable BHEmodel BHE_SegmentMatrix[nBHEelementsZ,nBHEelementsR] constrainedby MoBTES.Components.Sources.Geothermal.BaseClasses.BHEsegment_partial(
			segmentLength=GLOBAL_elementHeightsMatrix[iBHEheadElement:iBHEbottomElement, 1:nBHEelementsR],
			Tinitial=GLOBAL_TinitialMatrix[iBHEheadElement:iBHEbottomElement, 1:nBHEelementsR],
			each medium=medium,
			each BHEdata=BHEdata,
			groutData(
				rho=BHE_groutDataMatrix[1],
				cp=BHE_groutDataMatrix[2],
				lamda=BHE_groutDataMatrix[3]),
	...
	public
		...
		replaceable parameter Parameters.Settings.SettingsDefault settingsData constrainedby MoBTES.Parameters.Settings.SettingsPartial "Numerical settings" annotation(Dialog(
			...
			tab="Modeling"));
	protected
		replaceable GLOBALmodel GLOBAL_elementMatrix[nElementsZ,nElementsR] constrainedby MoBTES.Components.Ground.BaseClasses.GlobalElement_partial(
			groundData(
				rho=GLOBAL_groundDataMatrix[1],
				cp=GLOBAL_groundDataMatrix[2],
				lamda=GLOBAL_groundDataMatrix[3]),
			elementHeight=transpose(fill(heights, nElementsR)),
			innerRadius=GLOBAL_innerRadiusMatrix,
			outerRadius=GLOBAL_outerRadiusMatrix) "Global problem modeling approach" annotation(
			Placement(transformation(extent={{50,15},{70,35}})),
			Dialog(
				group="Global problem",
				tab="Modeling"));
		replaceable LOCALmodel LOCAL_elementMatrix[nBHEelementsZ,nBHEelementsR] constrainedby MoBTES.Components.Ground.BaseClasses.LocalElement_partial(
			groundData(
		...
		Modelica.Thermal.HeatTransfer.Components.HeatCapacitor GLOBAL_groundCapacitiesSide[iBHEbottomElement-iBHEheadElement+1,nElementsR-nBHEelementsR](
			...
			T(
				...
				each fixed=true)) "Thermal capacities of global model elements beside the storage" annotation(Placement(transformation(extent={{20,40},{40,60}})));
		Modelica.Thermal.HeatTransfer.Components.HeatCapacitor GLOBAL_groundCapacitiesBottom[nElementsZ-iBHEbottomElement,nElementsR](
			...
			T(
				...
				each fixed=true)) "Thermal capacities of global model elements below the storage" annotation(Placement(transformation(extent={{20,40},{40,60}})));
		Modelica.Thermal.HeatTransfer.Sources.FixedTemperature GLOBAL_outerBC[nElementsZ](T=GLOBAL_TinitialMatrix[:, 1]) "Fixed temperature boundary condition at the model side" annotation(Placement(transformation(extent={{95,15},{75,35}})));
		Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow GLOBAL_innerBC[nElementsZ](each Q_flow=0) "Fixed heat flow boundary condition at the inner model boundary" annotation(Placement(transformation(extent={{20,15},{40,35}})));
	...
	equation
		/*Outputs---------------------------------------------*/
		TflowArray = if nSeries == 1 then sum(BHE_SegmentMatrix[1, r].topFlowPort.h / medium.cp * nBHEsPerRingVector[r] / nBHEs for r in 1:nBHEelementsR) else BHE_SegmentMatrix[1, 1].topFlowPort.h / medium.cp;
		TreturnArray = if nSeries == 1 then sum(BHE_SegmentMatrix[1, r].topReturnPort.h / medium.cp * nBHEsPerRingVector[r] / nBHEs for r in 1:nBHEelementsR) else BHE_SegmentMatrix[1, nBHEelementsR].topReturnPort.h / medium.cp;
		...
		/*diagnostics*/
		assert(GLOBAL_elementMatrix[iBHEheadElement + integer((iBHEbottomElement + iBHEheadElement) / 2), nElementsR].groundCenterPort.T < GLOBAL_TinitialMatrix[iBHEheadElement + integer((iBHEbottomElement + iBHEheadElement) / 2), nElementsR] + 1, "Temperature of outer model boundary increased by more than 1K --> increase radial model size", AssertionLevel.warning);
		assert(GLOBAL_elementMatrix[nElementsZ, 1].groundCenterPort.T < GLOBAL_TinitialMatrix[nElementsZ, 1] + 1, "Temperature at model bottom increased by more than 1K --> increase model depth", AssertionLevel.warning);
		...
		connect(GLOBAL_elementMatrix[z,nElementsR].outerHeatPort,GLOBAL_outerBC[z].port) annotation(Line(points=0));
		  /*GLOBAL to GLOBAL*/
		...
		for r in 1:nElementsR loop
		  ...
		  connect(weatherPort.Tambient, GLOBAL_topBC[r].T);
		  connect(TsurfaceAverage.y, GLOBAL_topBC[r].T);
		  
		connect(GLOBAL_topBC[r].port,GLOBAL_elementMatrix[1,r].topHeatPort) annotation(Line(points=0));
		  
		connect(GLOBAL_elementMatrix[nElementsZ,r].bottomHeatPort,GLOBAL_bottomBC[r].port) annotation(Line(points=0));
		  /*GLOBAL to GLOBAL*/
		...
		connect(GLOBAL_groundCapacitiesTop[z,r].port,GLOBAL_elementMatrix[z,r].groundCenterPort) annotation(Line(points=0));
		  end for;
		end for;
		/*beside storage*/
		...
		if nSeries == 1 then
			connect(HYDR_pump.flowPort_b,BHE_Heads[r].flowPort);
			connect(BHE_Heads[r].returnPort,returnPort);
		else
			if r==1 then
				connect(HYDR_pump.flowPort_b,BHE_Heads[r].flowPort);
			else
				connect(BHE_Heads[r-1].returnPort,BHE_Heads[r].flowPort) annotation(Line(points=0));
			end if;
			if r==nBHEelementsR then
				connect(BHE_Heads[nBHEelementsR].returnPort,returnPort);
			end if;
		end if;
			  	connect(BHE_Heads[r].BHEflow, BHE_SegmentMatrix[1, r].topFlowPort);
		connect(BHE_SegmentMatrix[1, r].topReturnPort, BHE_Heads[r].BHEreturn);
		  
				
		/*BHE segment to BHE segment & BHE segment to LOCAL elements*/	
		  for z in 1:nBHEelementsZ loop
		    connect(BHE_SegmentMatrix[z, r].Radvective,BHE_Heads[r].Radvective);
...
<h4>General</h4>
<p>Modelica Borehole Thermal Energy Storage model (MoBTES)<a href=\"modelica://MoBTES.UsersGuide.References\">[Formhals2020]</a>: The model consists of 3 sub-models (see Fig. 1):</p>
<ol>
<li>Global model</li>
<li>Local model</li>
<li>BHE model</li>
...
<table border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
  ...
  <tr>
    <td>
      <img src=\"modelica://MoBTES/Utilities/Images/BTESstructure.png\" width=\"700\">
    </td>
  </tr>
</table>
...
<table border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
  ...
  <tr>
    <td>
   <img src=\"modelica://MoBTES/Utilities/Images/BHEspacing.png\" width=\"500\">
    </td>
  </tr>
</table>
